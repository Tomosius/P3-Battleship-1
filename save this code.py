...
def inputShipCheck(x :int, y :int, align :str, map :list, length : int):
    """
...
def searchMapForBiggestShip(map :list, shipSize :int, cpuShootCoordinatesLog :list):
    ...
    while True:  # An outer loop to keep generating new coordinates until unique ones are found
        ...
        if [coordinatesX, coordinatesY] not in cpuShootCoordinatesLog:
            print(f" this is complete log of all cpu shooting coordinates : {cpuShootCoordinatesLog}")
            print(
...
def playGame():
    ...
    while result is None:
        # Here, I am assuming you would like the CPU to play against itself.
        # If you want the player to take a turn, you would need to add a function call for the player's move.
